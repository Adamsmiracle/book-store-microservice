...
@DataJpaTest(
    properties = {
      ...
      // We use Testcontainers to run a PostgreSQL database in a Docker container for our tests. The
      // "jdbc:tc" URL prefix tells Spring Boot to use Testcontainers to start a PostgreSQL
      // container and connect to it. The "postgresql:16-alpine" part specifies the Docker image to
      // use for the PostgreSQL container, and the "///bookstore" part specifies the name of the
      // database to create in the container.
...
// the container lifecycle and datasource bean are managed centrally instead of being repeated in
// each
// test class.
